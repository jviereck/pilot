...
function changeCommandKeyBinding(commandName, bindKey) {
    var command = commands[commandName];
    removeCommandKeyBinding(commandName);
    if (bindKey) {
...
function removeCommandKeyBinding(commandName) {
    var command = commands[commandName];
    if (!command) {
        throw "Can't remove unkown command " + commandName;
    }

    // Check if the command is bound to keys at all.
    if (!command.bindKey) {
        return;
    }
    ...
    var hashId;
    var key;
    for (hashId in commandKeyBindingFunc) {
        var hashIdContent = commandKeyBindingFunc[hashId];
        for (key in hashIdContent) {
            var keyArray = hashIdContent[key];
            for (var i = 0; i < keyArray.length; i++) {
                if (keyArray[i].command.name == commandName) {
                    keyArray.splice(i, 1);
                    // Need to test the same index again as we removed one
                    // entrie.
                    i--;
                }
            }
    ...
    var bindings = commmandKeyBinding[command.bindKey.sender];
    for (hashId in bindings) {
        for (key in bindings[hashId]) {
            if (bindings[hashId][key] === command) {
                delete bindings[hashId][key];
            }
        }
    }
...
exports.removeCommandKeyBinding = removeCommandKeyBinding;
exports.changeCommandKeyBinding = changeCommandKeyBinding;
